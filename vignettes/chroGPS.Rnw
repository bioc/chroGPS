...
Or for instance, if we would like to generate an interactive, light and shareable view of our 2D or 3D chroGPS map using PlotLy, we can use the following code as an example.
...
# colnames(xx) <- c('x','y','z')
# xx$color <- as.factor(s2names$Color)    
# ids <- as.character(s2names$Factor)
# p <- plot_ly(xx,x=~x,y=~y,z=~z,color=~color,colors=col2hex(levels(xx$color)),text=ids) %>%
# add_markers() %>%
# layout(title='S2 3D plotLy example',
#  scene = list(xaxis = list(title = 'X'),
#  yaxis = list(title = 'Y'),
#  zaxis = list(title = 'Z')))
#  htmlwidgets::saveWidget(as_widget(p), "chrogps_3d_plotly.html") # export for offline use, requires pandoc installed
@
